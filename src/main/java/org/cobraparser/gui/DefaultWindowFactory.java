...
import org.cobraparser.settings.GeneralSettings;
...
public class DefaultWindowFactory implements WindowFactory {
  ...
  private static final String DEFAULT_ICON_URL = "res:/images/lobo_icon128.png";
  ...
  private ImageIcon getImageIcon(final String urlOrPath, final UserAgentContext uaContext) {
    synchronized (this) {
      ...
      if (icon == null) {
        try {
          final byte[] imageBytes = org.cobraparser.request.RequestEngine.getInstance().loadBytes(urlOrPath, uaContext);
